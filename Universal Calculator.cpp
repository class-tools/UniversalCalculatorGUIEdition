...
string s;
double in1,in3,in4,in5,in6,hs;//in1、in2、in3、in4、in5、in6、in7、in8用于输入，不可调用。其余变量为共享变量。
...
int main()
{
	...
	cout << "取模：被模数、%和模数，按Enter键输出被模数除以模数的余数。" << endl;
	...
	cout << "注意事项：请尽量不要严重错误的数据（例如一串乱码），以免影响您的体验。建议在英文输入法下使用。" << endl;
	while(1)
	{
		fflush(stdin);
		cin >> in1 >> in2;
